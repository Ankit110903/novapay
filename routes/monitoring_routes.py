...
def metrics():
    ...
    db_res = check_db_health()
    sys_res = get_system_telemetry()
    sys_res = {key: (float('nan') if value is None else value) for key, value in sys_res.items()}
    uptime = time.time() - START_TIME
...
def api_manual_failover():
    ...
    try:
        ...
        current_active = response.json()["active_cloud"]
        if current_active == target_server:
            return jsonify({"success": False, "error": {"code": "ALREADY_ACTIVE", "message": "Target is already active."}}), 400
        target_url = os.environ.get(f"{target_server.upper()}_URL", "")
        if not target_url:
            raise ValueError("Target URL is not configured")
        probe = requests.get(f"{target_url.rstrip('/')}/health", timeout=10)
        probe.raise_for_status()
        if probe.json().get("status") != "healthy":
            raise ValueError("Target node is not healthy")
        start_time = time.monotonic()
        response = requests.post(
            f"{PROXY_URL}/proxy/update-routing", json={"active": target_server},
            headers={"Authorization": f"Bearer {os.environ.get('PROXY_CONTROL_TOKEN', '')}"}, timeout=5)
        response.raise_for_status()
        if response.json().get("active_cloud") != target_server:
            raise ValueError("Proxy did not confirm the requested target")
        rto = time.monotonic() - start_time
        proxy_ok = True
    except Exception as e:
        print(f"[FAILOVER ERROR] Failed to signal proxy routing: {e}")
        return jsonify({"success": False, "error": {"code": "FAILOVER_FAILED", "message": "Target health or proxy routing could not be verified."}}), 503
