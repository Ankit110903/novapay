...
def api_deposit():
    ...
    if not current_app.config.get("ALLOW_DEMO_DEPOSITS"):
        return jsonify({"success": False, "error": {"code": "DEMO_DISABLED", "message": "Demo deposits are disabled on this deployment."}}), 403
    data = request.get_json() or {}
    to_acc = data.get("to_account_id")
    amount = data.get("amount", "0.00")
