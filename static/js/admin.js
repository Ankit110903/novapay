...
function updateNodeTelemetry(node, data) {
    ...
    const stale = !data?.created_at || Date.now() - Date.parse(data.created_at) > 120000;
    const status = stale ? "STALE" : String(data?.status || "UNKNOWN").toUpperCase();
    ...
    setText(`${node}-cpu`, !stale && data?.cpu_usage != null ? `${cpu.toFixed(1)}%` : "Unavailable");
    setText(`${node}-mem`, !stale && data?.memory_usage != null ? `${memory.toFixed(1)}%` : "Unavailable");
