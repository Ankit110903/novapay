...
def check_instance_health(url):
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            if data.get("status") != "healthy" or data.get("db") != "connected":
                raise ValueError("Node reports degraded health or disconnected database")
            return {
                "status": "UP",
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (
                    ...
                    Decimal(str(result["cpu"])) if result["cpu"] is not None else None,
                    Decimal(str(result["memory"])) if result["memory"] is not None else None,
                    result["sessions"],
                    region_label,
                    result["error"]
                )
...
def trigger_failover(from_server, to_server, reason):
    ...
    start_time = time.time()
    target_url = AWS_URL if to_server == "aws" else AZURE_URL
    if check_instance_health(target_url)["status"] != "UP":
        print("[FAILOVER SKIPPED] Standby is not healthy; retaining current route.")
        return False
    ...
    try:
        resp = requests.post(f"{PROXY_URL}/proxy/update-routing", json={"active": to_server},
                             headers={"Authorization": f"Bearer {os.environ.get('PROXY_CONTROL_TOKEN', '')}"}, timeout=5)
        proxy_ok = resp.status_code == 200 and resp.json().get("active_cloud") == to_server
    except Exception as e:
        proxy_ok = False
    ...
    if not proxy_ok:
        print("[FAILOVER FAILED] Proxy did not confirm the switch; retaining current state.")
        return False
    ...
    try:
        ...
        notify_admins(
            ...
            f"NovaPay routing switched from {from_server.upper()} to {to_server.upper()} in {rto:.3f} seconds after detection. Reason: {reason}."
        )
    except Exception as e:
...
def main_loop():
    ...
    while True:
        # Manual overrides and daemon restarts must use the proxy's actual state.
        try:
            routing = requests.get(f"{PROXY_URL}/proxy/status", timeout=5)
            routing.raise_for_status()
            active = routing.json().get("active_cloud")
            if active in ("aws", "azure"):
                current_active = active
        except requests.RequestException as e:
            print(f"[DAEMON ERROR] Could not read proxy state: {e}")
        ...
        if os.environ.get("PROCESS_SCHEDULED_TRANSFERS", "false").lower() == "true":
