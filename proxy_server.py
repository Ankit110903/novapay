...
def load_active_cloud():
    """Restore a confirmed route after a single-worker gateway restart."""
    state_file = os.environ.get("PROXY_STATE_FILE")
    if state_file and Path(state_file).exists():
        active = json.loads(Path(state_file).read_text())["active_cloud"]
        if active not in ROUTING_TABLE:
            raise ValueError("Invalid persisted cloud route")
        return active
    return ACTIVE_CLOUD

ACTIVE_CLOUD = load_active_cloud()

def persist_active_cloud(active):
    state_file = os.environ.get("PROXY_STATE_FILE")
    if state_file:
        target = Path(state_file)
        target.parent.mkdir(parents=True, exist_ok=True)
        temporary = target.with_suffix(".tmp")
        temporary.write_text(json.dumps({"active_cloud": active}))
        temporary.replace(target)

# Persistent session pool for outbound cloud requests
# Do not share a requests.Session cookie jar between bank customers.
...
def update_routing():
    ...
    global ACTIVE_CLOUD
    token = os.environ.get("PROXY_CONTROL_TOKEN", "")
    ...
    with ROUTING_LOCK:
        try:
            persist_active_cloud(new_target)
        except OSError:
            app.logger.exception("Could not persist routing state")
            return jsonify({"success": False, "error": "Routing state could not be saved"}), 503
...
def proxy(path):
    ...
    excluded_req_headers = ["host", "content-length", "connection", "transfer-encoding", "keep-alive", "upgrade", "proxy-authorization", "proxy-authenticate", "te", "trailer"]
    ...
    try:
        ...
        resp_headers = []
        for k in resp.raw.headers:
            k_lower = k.lower()
            if k_lower in excluded_resp_headers:
                continue
            # Preserve separate Set-Cookie headers from the upstream.
            for v in resp.raw.headers.getlist(k):
                if k_lower == "location":
